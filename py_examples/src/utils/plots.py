...
def plotTrainingSummary(workbook, trainingSummary):
    ...
    st1 = uuid.uuid1()
    st2 = uuid.uuid1()
    workbook.create_sheet(index=None, title=st1)
    workbook.create_sheet(index=None, title=st2)
    sheet1 = workbook[st1]
    sheet2 = workbook[st2]
    for i, (name, summary) in enumerate(trainingSummary.items()):
        sheet1.cell(row=1, column=i + 1).value = name
        for j, value in enumerate(summary['loss']):
            sheet1.cell(row=j + 2, column=i + 1).value = value
        sheet2.cell(row=1, column=i + 1).value = name
        for j, value in enumerate(summary['val_loss']):
            sheet2.cell(row=j + 2, column=i + 1).value = value
